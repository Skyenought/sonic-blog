...
func NewServer(param ServerParams, lifecycle fx.Lifecycle) *Server {
	...
	router := hzserver.New(
		...
		hzserver.WithDisablePrintRoute(true),
		hzserver.WithCustomValidator(vd),
	)
...
func (s *Server) Run(ctx context.Context) error {
	go func() {
		if err := s.Router.Run(); err != nil && err != http.ErrServerClosed {
...
func (s *Server) wrapHandler(handler wrapperHandler) hzapp.HandlerFunc {
...
type wrapperHTMLHandler func(_ctx context.Context, ctx *hzapp.RequestContext, model template.Model) (templateName string, err error)
...
func (s *Server) wrapHTMLHandler(handler wrapperHTMLHandler) hzapp.HandlerFunc {
	return func(_ctx context.Context, ctx *hzapp.RequestContext) {
...
func (s *Server) wrapTextHandler(handler wrapperHTMLHandler) hzapp.HandlerFunc {
	return func(_ctx context.Context, ctx *hzapp.RequestContext) {
		model := template.Model{}
		templateName, err := handler(_ctx, ctx, model)
		if err != nil {
...
func (s *Server) handleError(_ctx context.Context, ctx *hzapp.RequestContext, err error) {
	status := xerr.GetHTTPStatus(err)
